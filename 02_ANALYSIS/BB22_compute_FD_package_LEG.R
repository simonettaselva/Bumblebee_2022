...
library(reshape2)
wide <- dcast(BB22_full.ab, BB22_full.ab[[j]] ~ plant.species, value.var="abundance")[,-1]
rownames(wide)  <- levels(BB22_full.ab[[j]])
wide[is.na(wide)] <- 0
...
write.csv(assign(paste("df.FD",i,j, sep = "_"), df.FD[, c(1,3,5,7)]), file = paste("./FD/leg/FD_package_", i, "_", j, ".csv", sep = ""))
