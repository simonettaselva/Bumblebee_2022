################################################
# Statistical Models script INDIVIDUALS LEG
# by Simonetta Selva
#
...
BB22.fun.ID <- read_csv("./FD/leg/FD_package_B.pascuorum_ID.short.csv")%>% 
...
BB22.ID <- merge(BB22.bb.traits.sp, BB22.fun.ID, by  = "ID")
...
annotate_figure(plot, top = text_grob("B.pascuorum: species richness and funtional diversity across landscapes", 
                                      face = "bold", size = 22))
ggsave("./functional diversity/pasc_ID/leg/FD_B.pascuorum_ID.png", width = 6, height = 24)
setwd(input)
...
for (i in metrics) {
  ...
  plot4 <- ggarrange(a1,a2,a3,a4,a5,a6,a7,a8,a9, # arrange to plots nicely and export them 
                     ...
                     common.legend = TRUE)
  annotate_figure(plot4, top = text_grob(paste("LEG B.pascuorum: comparison of ", i, " and traits across landscapes", sep = ""),
                                         face = "bold", size = 22))
  ggsave(paste("./functional diversity/pasc_ID/leg/FD_pasc_corr_", i, "_BBtraits_landscapes.png", sep = ""), width = 16, height = 8)
  setwd(input)
} # end loop i
...
for (i in metrics) {
  ...
  setwd(output)
  ggsave(paste("./functional diversity/pasc_ID/leg/training and predicting/linear model/", i, "_lm_var_imp_pasc.png", sep = ""), width = 8, height = 8)
  setwd(input)
  ...
  ggsave(paste("./functional diversity/pasc_ID/leg/training and predicting/linear model/", i, "_lm_prediction_pasc.png", sep = ""), width = 8, height = 8)
  setwd(input)
} # end i loop
...
for (i in metrics) {
  ...
  setwd(output)
  ggsave(paste("./functional diversity/pasc_ID/leg/training and predicting/random forest/", i, "_rf_var_imp_pasc.png", sep = ""), width = 8, height = 8)
  setwd(input)
  ...
  ggarrange(gg_test, gg_train, ncol = 2, nrow = 1,
            labels = c("A", "B"), common.legend = TRUE, legend = "right")
  ggsave(paste("./functional diversity/pasc_ID/leg/training and predicting/random forest/", i, "_rf_prediction_pasc.png", sep = ""), width = 8, height = 8)
  setwd(input)
